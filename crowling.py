...
for checkbox in checkboxes:
    if checkbox.is_selected(): # 체크된 생태라면
...
for checkbox in checkboxes:
    ...
    label = parent.find_element(By.TAG_NAME, 'label') # label element 찾기
...
for idx in range(1, 40): # 1 이상 40 미만 페이지 반복
    ...
    df.dropna(axis = 'index', how = 'all', inplace = True) # 행 전체가 결측치일 때 지우기
